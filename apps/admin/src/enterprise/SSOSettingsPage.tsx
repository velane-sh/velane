...
export default function SSOSettingsPage() {
  ...
  return <div className="max-w-3xl space-y-6">
    ...
    <form onSubmit={save} className="space-y-4 rounded-lg border border-gray-200 bg-white p-6">
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium text-gray-700">Protocol<select className={`${inputClass} mt-1`} value={value.protocol} onChange={(e) => setValue({ ...value, protocol: e.target.value as 'oidc' | 'saml' })}><option value="oidc">OpenID Connect</option><option value="saml">SAML 2.0</option></select></label><label className="text-sm font-medium text-gray-700">Display name<input className={`${inputClass} mt-1`} required value={value.display_name ?? ''} onChange={(e) => setValue({ ...value, display_name: e.target.value })} /></label></div>
      ...
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium text-gray-700">JIT default role<select className={`${inputClass} mt-1`} value={value.default_role} onChange={(e) => setValue({ ...value, default_role: e.target.value as 'invoke' | 'manage' })}><option value="invoke">Invoke</option><option value="manage">Manage</option></select></label><label className="text-sm font-medium text-gray-700">Break-glass admin<select className={`${inputClass} mt-1`} value={value.break_glass_user_id ?? ''} onChange={(e) => setValue({ ...value, break_glass_user_id: e.target.value })}><option value="">Select before enforcement</option>{members.filter((m) => m.role === 'admin').map((m) => <option key={m.user_id} value={m.user_id}>{m.email}</option>)}</select></label></div>
